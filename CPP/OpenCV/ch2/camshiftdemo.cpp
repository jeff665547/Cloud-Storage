// source code from C:\opencv\sources\samples\cpp (default path: C:\Program Files\opencv)
// 彩色目標跟蹤: Camshift
// 程式的用法是根據滑鼠框選區域的色度光譜來進行攝影機讀入的視訊目標的跟蹤。
// 其主要採用CamShift演算法，
// 全稱是Continuously Adaptive Mean-SHIFT,
// 是對MeanShift演算法的改進 (連續自我調整的MeanShift演算法)
...
static void onMouse( int event, int x, int y, int, void* )
{
    if( selectObject )
    {
        selection.x = MIN(x, origin.x); // Takes the minimum of the inputs.
        selection.y = MIN(y, origin.y); 
        selection.width = std::abs(x - origin.x);
        selection.height = std::abs(y - origin.y);

        std::cout << (selection & cv::Rect(0, 0, image.cols, image.rows)) << std::endl;
        selection &= cv::Rect(0, 0, image.cols, image.rows); // A = A & B bitwise and operation 
        // std::cout << selection;
    }
...
int main( int argc, const char** argv )
{
    ...
    cv::Rect trackWindow;
    int hsize = 16;
    float hranges[] = {0,180}; // array of 2 elements
    ...
    cv::setMouseCallback( "CamShift Demo", onMouse, 0 );  // onMouse is defined in the front.
    ...
    for(;;) // the same as while(true){}
    {
        ...
        if( !paused )
        {
            ...
            if( trackObject )
            {
                ...
                cv::ellipse( image, trackBox, cv::Scalar(0,0,255), 3, CV_AA );
            }
        }
